...
bool SpaceGame::Initialize()
{
    ...
    m_engine->GetAudio().AddSound("pew.mp3");
...
void SpaceGame::Update(float dt)
{
    ...
    case eState::Game:
        ...
        if (m_spawnTimer <= 0)
        {
            ...
            auto enemy = std::make_unique<Enemy>(20000.0f, Transform{ { random(g_engine.GetRenderer().GetWidth()),  random(g_engine.GetRenderer().GetHeight()) }, 0, 2 }, enemyModel);
            enemy->SetDamping(1.0f);
            enemy->SetTag("Enemy");
            m_scene->AddActor(std::move(enemy));

            // create pickup
            /*auto* pickupModel = new Model{GameData::powerPoints, Color{1, 1, 1}};
